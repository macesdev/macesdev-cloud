...
<template>
  <div id="wrapper">
          ...
          <div class="px-6 py-8 lg:px-8">
            ...
            <div>
              <input
                ...
                id="upassword"
                placeholder="Anahtar"
                class="bg-gray-50 border border-gray-300 mb-4 text-gray-900 text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                required
              />
            </div>
                ...
                <div class="column">
                  <div class="flex items-center space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      >
                        {{ folder.author }} tarafından oluşturuldu.
                      </p>
                    </div>
                    <div
                      class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white"
                    >
                      <inspect @click="setFolder(folder.sid)" />
                    </div>
                  </div>
                </div>
                <div class="pt-3"></div>
    ...
    <div class="loadingScreen" v-show="!dataLoaded">
      <div v-show="authenticateWaiting">
        <div class="p-4 sm:ml-64">
          <section class="bg-white dark:bg-gray-900">
            <div class="">
              <div role="status">
                ...
                <p class="pt-3 text-base text-gray-900 dark:text-white">
                  Doğrulanıyor
                </p>
              </div>
            </div>
...
export default {
  ...
  mounted() {
    ...
    console.log(route);
    this.$server.get("/mcloud/data/get-folders").then((response) => {
  ...
  methods: {
    setFolder(selectedFid) {
      this.selectedFolder = selectedFid;
    ...
    authenticateFolder() {
      ...
      let req = {
        sid: this.selectedFolder,
        pass: this.authPassword,
      };

      this.dataLoaded = false;
      this.authenticateWaiting = true;

      this.$server.post("/mcloud/auth/get-access-key", req).then((response) => {
        console.log(response);

        if (response.data.status == "PASSWORD_NOT_MATCH") {
          if (
            confirm(
            ...
            this.dataLoaded = true;
            this.authenticateWaiting = false;
          } else {
            this.dataLoaded = true;
            this.authenticateWaiting = false;
          }
          ...
          this.$router.push({
            path: "/view_folder",
            query: { sid: this.selectedFolder, key: response.data.key },
          });
        }
      });
      // burda keyi alıcak ve view_folder'a data ile gönderecek. bu sayede auth işlemi bitmiş olacak.
    },
  },
};
</script>
